...
### Agent A — Go orchestration front-half + spawn security (RESULT)
- **A/F2 (NEGATIVE — clean, high conf):** NO attacker-request-string → argv/env on inference path. argv strings (--model,--mmproj,--lora,--cache-type,--spec-draft-model) are model-config-derived; request-Options that reach argv (NumCtx,NumBatch,NumGPU,NumThread,MainGPU,NumKeep) are numeric via strconv.Itoa; no shell (exec.Command llama_server.go:432). Command-injection RCE dead on inference path. (Modelfile create route is the only string-into-config path, separate scope.)
- **A/F1:** runner port no daemon↔runner auth (matches B3). Only Content-Type header set.
- **A/F3 (Low-Med, speculative):** refcount corruption on reload/swap — `processCompleted` (sched.go:382-390) does `runner.refCount--` on current s.loaded[key], not the runnerRef the request was assigned; refCount is uint → underflow (leak, never unloads) or drop in-use runner to 0 → unload mid-inference. Plus getRunner TOCTOU (reads s.loaded under lock, unlocks :204-206, useLoadedRunner increments :484 without lock → can hand back a just-Closed runner). Go-safe (no corruption), correctness/DoS.
- **A/F4 (Low, confirmed reachable):** unauth load/unload amplification DoS — keep_alive:0 (sessionDuration<=0 → kill subprocess each request) + option churn (num_ctx/num_gpu → needsReload true, sched.go:1441) → repeated expensive model load + VRAM realloc, no rate limit.
